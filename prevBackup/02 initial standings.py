...
try:    # обработка исключений для определения ошибки и записи ее в bug_file в блоке except
    ...
    import mod_Nat_tournaments  # модуль словаря нац турниров
    import mod_api_request      # модуль поиска информации в запросе
    import mod_cup_files        # модуль актуализации файлов кубков
    import mod_league_files        # модуль актуализации файлов нац лиг
        ...
        for ass_n in Ass_TournRateQuot:
            ...
            for tourn in Ass_TournRateQuot[ass_n]:
                ...
                if tourn[0].find("Cup") != -1:  # для всех кубковых турниров учитываются: незавершившийся турнир и предыдущий, если с его финала прошло <150 дней
                    mod_cup_files.func_cup_files(tourn[0], calc_date)   # актуализация файлов кубков
                    tourn[2] = 0        # изменение элемента на Rating
                    tourn.insert(3, 0)  # добавление элемента Quota
                    for tourn_file in os.listdir('tournaments/'):
                        ...
                        if tourn_file.find(tourn[0]) != -1 and tourn_file.find(tourn[1]) != -1 and tourn[1] == "prev":
                            with open("tournaments\\"+tourn_file, 'r') as f:
                                for line in f:
                                    if calc_date >= mod_api_request.PrevCupInfluence(line):  # если после финала прошло 150 дней и больше
                        ...
                        if tourn_file.find(tourn[0]) != -1 and tourn_file.find(tourn[1]) != -1 and tourn[1] == "curr":
                            with open("tournaments\\"+tourn_file, 'r') as f:
                                for line in f:
                                    if calc_date < mod_api_request.CupFirst(line):
                                        Del_tourn.append(tourn)
...
# после расчета initial standings запомнить все полученные итоговые словари в файлы

except: 
    with open("bug_files\\"+DateNowExc+".txt", 'w') as f:
        traceback.print_exc(file=f)     # создание файла ошибки с указанием файла кода и строки в нем